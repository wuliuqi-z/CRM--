...
<head>
	...
	$(function(){
	// 	给全选或者取消按钮添加单击事件
		$("#selectCancelAll").click(function () {
			$("#clueList input[type='checkbox']").prop("checked",$("#selectCancelAll")[0].checked);

		})
	// 	给每一个虚假的选择按钮添加单击事件
		$("#clueList").on("click","input[type='checkbox']",function (){
			$("#selectCancelAll")[0].checked=$("#clueList input[type='checkbox']:checked").size()==$("#clueList input[type='checkbox']").size()
		})
	// 	给查询按钮添加单击事件
		$("#queryBtn").click(function (){
			queryClueByConditionForPage(1,$("#dom_page").bs_pagination('getOption','rowsPerPage'))
		})
	// 	页面加载完毕之后，立即查询数据
	...
	// 	给创建按钮添加单击事件
		...
		// 	首先先对模态窗的值先进行初始化
			$("#createForm")[0].reset();//表单的dom对象自带的重置
	...
	function queryClueByConditionForPage(pageNo,pageSize=10){
		$("#selectCancelAll")[0].checked=false
		let company =$.trim($("#queryCompany").val());
		let fullname = $.trim($("#queryName").val());
		let mphone = $.trim($("#queryMPhone").val());
		let owner = $.trim($("#queryOwner").val());
		let phone = $.trim($("#queryPhone").val());
		let source = $.trim($("#querySource").val());
		let status = $.trim($("#queryStatus").val());
		$.ajax({
			...
			success:function (data) {
				...
				$.each(data.clueList,function(index,clue){
					htmlStr+=`<tr>
								...
								<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href='workbench/clue/lookDetailClue.do?clueId=${'${clue.id}'}';">${"${clue.fullname}${clue.appellation}"}</a></td>
								<td>${"${clue.company}"}</td>
								<td>${'${clue.mphone}'}</td>
								<td>${'${clue.phone}'}</td>
								<td>${'${clue.source}'}</td>
				...
				console.log("222222222222"+totalRow)
				$("#clueList").html(htmlStr)
				let totalPages=Math.ceil(totalRow/pageSize)
			// 	调用分页函数，现实分页信息
			// 	设置分页插件参数
				$("#dom_page").bs_pagination({
					totalPages:totalPages,
					currentPage:pageNo,
					rowsPerPage:pageSize,//每页默认显示条数
					totalRow:totalRow,
					visiblePageLinks:5,
					showGoToPage:true,
		...
		<div class="modal-dialog" role="document" style="width: 90%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="updateClueBtn">保存</button>
				</div>
			</div>
		</div>
	</div>
				...
				<table class="table table-hover">
					<thead>
						<tr style="color: #B3B3B3;">
							<td><input type="checkbox" id="selectCancelAll"/></td>
							<td>名称</td>
							<td>公司</td>
							<td>公司座机</td>
							<td>手机</td>
...
<%--                        </tr>--%>
			...
			<div id="dom_page" style="height: 50px; position: relative;top: 60px;">
