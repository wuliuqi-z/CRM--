...
<head>
	...
	$(function(){
		...
		// 给解除关联按钮添加单击事件
		$("#bindedActivityTbody").on("click","a",function (){

			let activityId=$(this).attr("activityId")
			let cludId='${clue.id}'
			console.log(activityId+cludId)
			let a=$(this)
			$.ajax({
		...
		// 	获取所有状态为选中的市场活动的id
			let checkeds=$("#activityTable input[type='checkbox']:checked")
			//拼接字符串
			let str=""
			$.each(checkeds,function (index,checked){
				str+="activityId="+checked.value+"&"
			})
			str+="clueId="+'${clue.id}'
			console.log(str);
			$.ajax({
				...
				success:function (data){
					if(data.code=="1"){
						...
						$("#bindedActivityTbody").prepend(htmlStr)
		...
		$("#searchForm input").keyup(function (event){
			...
			$.ajax({
				...
				success:function (data){
					...
					$("#activityTable tbody").html(htmlStr)
				}
			})
		})
...
<%--						</c:forEach>--%>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>${clue.fullname}${requestScope.clue.appellation}<small>${clue.company}</small></h3>
		</div>
...
<%--			为了当需要删除div的时候方便--%>
			<div class="remarkDiv" style="height: 60px;" id="div_${clueRemark.id}">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					...
					<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.fullname}${clue.appellation}-${clue.company}</b> <small style="color: gray;"> ${clueRemark.editFlag==0?clueRemark.createTime:clueRemark.editTime} 由${clueRemark.editFlag==0?clueRemark.createBy:clueRemark.editBy}${clueRemark.eidtFlag==0?'创建':'修改'}</small>
					<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
						<a class="myHref" href="javascript:void(0);" remark-id="${clueRemark.id}"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a class="myHref" href="javascript:void(0);" remark-id="${clueRemark.id}"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>
					</div>
				</div>
			</div>
		</c:forEach>
